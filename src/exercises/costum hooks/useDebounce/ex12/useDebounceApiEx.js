...
function useDebounceApiEx(value, delay) {
    ...
    useEffect(() => {
        ...
        // so while the user typing, the setTime can't run yet, the clearTime out runs each time and prevent duplicates, or deisplaying each character we typing each time, it's clear it in each time and only on the and point when the user stop typing and use effect get into the setTimeout afet 1 or 2 or whatever seconds, he can set our value that full whole value that he see now in the input, then do with that request or whatever. i need to understand if the user now stop typing, he typed 'a' clear, typed 'b', clear 'b' stopped, 2 seconds pass, the setTimeout run with the full value that he see tight now it the input
        // 
